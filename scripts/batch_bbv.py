...
def get_bbv(linux_path, bbv_path):
    print("try run " + linux_path)
    start_time = time.time()
    try:
        os.makedirs(os.path.dirname(bbv_path), exist_ok=True)
        log_file = open(os.path.join(os.path.dirname(bbv_path), "bbv.log"), "w")
        bblk_path = os.path.join(os.path.dirname(bbv_path), "bblk.txt")
        cmd = [la_emu_path, "-m", "16", "-z", "-k", linux_path, "-p", f"{libbbv},bblk={bblk_path},bbv={bbv_path},gz=1,interval={sm_interval},ibar0x40=1"]
        print(f"cmd={cmd}")
...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_cores) as executor:
        results = executor.map(get_bbv, linux_paths, bbv_paths)
