...
def get_bblk(linux_path, bblk_path):
    print("try run " + linux_path)
    start_time = time.time()
    try:
        os.makedirs(os.path.dirname(bblk_path), exist_ok=True)
        log_file = open(os.path.join(os.path.dirname(bblk_path), "bblk.log"), "w")
        cmd = [la_emu_path, "-m", "16", "-z", "-k", linux_path, "-p", f"{libbblk},bblk={bblk_path},ibar0x40=1"]
        print(f"cmd={cmd}")
...
def main():
    ...
    bblk_paths = [os.path.join(simpoint_outdir, file.rsplit('.', 1)[0], "bblk.txt") for file in os.listdir(linux_outdir)]
    
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_cores) as executor:
        results = executor.map(get_bblk, linux_paths, bblk_paths)
